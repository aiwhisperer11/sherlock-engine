...
export default function SherlockInvestigationView({ investigation }: SherlockInvestigationViewProps) {
  ...
  return <article className="space-y-9">
    <header>
      ...
      <div className="mt-1 flex flex-wrap items-center gap-3"><h1 className="text-3xl font-bold">{investigation.meta.case_title}</h1><span className="rounded-full border border-zinc-300 px-2 py-1 text-sm dark:border-zinc-700">Iteration {investigation.meta.iteration}</span></div>
    ...
    <section aria-labelledby="hypotheses-heading"><h2 id="hypotheses-heading" className="text-xl font-semibold">Hypotheses</h2><div className="mt-3 grid gap-3 lg:grid-cols-2">{currentHypotheses.map((hypothesis) => <HypothesisCard key={hypothesis.id} hypothesis={hypothesis} />)}</div>{graveyard.length > 0 && <div className="mt-6 border-t border-zinc-300 pt-6 dark:border-zinc-700"><h3 className="text-lg font-semibold">Hypothesis Graveyard</h3><div className="mt-3 grid gap-3 lg:grid-cols-2">{graveyard.map((hypothesis) => <HypothesisCard key={hypothesis.id} hypothesis={hypothesis} />)}</div></div>}</section>
    ...
    <section aria-labelledby="wald-heading"><h2 id="wald-heading" className="text-xl font-semibold">Missing evidence (WALD)</h2><div className="mt-3 grid gap-3 md:grid-cols-3">{criticalities.map((criticality) => <div key={criticality} className="rounded border border-zinc-200 p-4 dark:border-zinc-800"><h3 className="font-semibold capitalize">{criticality}</h3><ul className="mt-2 space-y-3 text-sm">{investigation.missing_evidence.filter((item) => item.criticality === criticality).map((item) => <li key={item.id}><p className="font-medium">{item.id}: {item.description}</p><p className="mt-1 text-zinc-600 dark:text-zinc-400">Impact: {item.impact_if_found}</p><p className="mt-1 text-xs text-zinc-500">Hypotheses: {item.related_hypothesis_ids.join(", ") || "none"}</p></li>) || <li>None.</li>}</ul></div>)}</div></section>
