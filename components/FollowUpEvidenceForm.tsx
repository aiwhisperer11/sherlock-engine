...
export default function FollowUpEvidenceForm({ previousSnapshot, onSnapshot }: FollowUpEvidenceFormProps) {
  ...
  return <section aria-labelledby="new-evidence-heading" className="rounded-lg border border-sky-300 bg-sky-50 p-5 dark:bg-sky-950/20">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 id="new-evidence-heading" className="text-xl font-semibold">New evidence</h2><p className="text-sm text-zinc-600 dark:text-zinc-400">Re-investigate iteration {previousSnapshot.meta.iteration + 1} from the latest snapshot.</p></div><button type="button" onClick={() => setEvidence(followUpExample.map((item) => ({ ...item })))} className="rounded border border-sky-400 px-3 py-2 text-sm">Load follow-up example</button></div>
